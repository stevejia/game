...
import com.futures.manager.PersonAuthRecordManager;
import com.futures.manager.PigAwardLogManager;
import com.futures.manager.RevealLuckyManager;
import com.futures.mapper.PigReservationMapper;
...
@Slf4j
@Service
public class PigReservationServiceImpl extends CrudServiceSupport<PigReservationMapper, PigReservation>
		implements PigReservationService {
	...
	@Override
	public IPage<PigReservationModifyDto> queryPigReservation(IPage<PigReservationModifyDto> page,
			PigReservationModifyDto dto) {
	...
	public BaseResponse reservat(ReservatDto param) {
		...
		Date resDate = DateUtils.parse(DateUtils.format(goods.getStartTime(), DateUtils.DEFAULT_TIME_FORMAT),
				DateUtils.DEFAULT_TIME_FORMAT);
...
//				"预约木材:" + goods.getGoodsName(), IncomeTypeEnum.RESERVAT, goods);
		...
		PigAwardLog awardLog = awardLogService.getOne(new QueryWrapper<PigAwardLog>()
				.eq("open_result", CommEnum.FALSE.getCode()).eq("pig_id", param.getGoodsId()));
		if (null == awardLog) {
			// 初始化中奖记录
...
//		Set<Long> users;
		...
		Date nowDate = DateUtils.parse(DateUtils.format(DateUtils.currentDate(), DateUtils.DEFAULT_TIME_FORMAT),
				DateUtils.DEFAULT_TIME_FORMAT);
		Date resDate = DateUtils.parse(DateUtils.format(goods.getEndTime(), DateUtils.DEFAULT_TIME_FORMAT),
				DateUtils.DEFAULT_TIME_FORMAT);
		if (resDate.before(nowDate)) {
...
//        }
	...
	public String openDate(PigGoods goods) {
		...
		String fristDate = nowDateSuffix + StringUtils.SPACE
				+ DateUtils.format(goods.getEndTime(), DateUtils.DEFAULT_TIME_FORMAT);
		String secondDate = nowDateSuffix + StringUtils.SPACE
				+ DateUtils.format(goods.getSecondEndTime(), DateUtils.DEFAULT_TIME_FORMAT);
		// 找出最近的一次开奖时间
		...
		if (DateUtil.before(fristDate)) {
			...
			Date date = org.apache.commons.lang3.time.DateUtils
					.addDays(DateUtils.parse(fristDate, DateUtils.DEFAULT_DATE_TIME_FORMAT), 1);
