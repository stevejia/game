...
public class DateUtils {
    ...
    public static final String DATETIME_DEFAULT_FORMAT_DATE = "yyyyMMddHHmmss";
    // 默认时间格式
     ...
     * @return Date
     */
    public static String getNowDateToStringStr() {
        return new DateTime().toString(DATETIME_DEFAULT_FORMAT_DATE);
    }
    ...
    public static Integer getDiffDays(Date startDate, Date endDate) {
        if (jodd.util.StringUtil.isEmpty((CharSequence) startDate) || jodd.util.StringUtil.isEmpty((CharSequence) endDate)) {
            return 0;
        }
    ...
    /**
     * @Description: 计算两个日期相隔多少秒(返回时间戳)
     * @Param: [start, end]
     * @return: int
     * @Author: cx
     * @Date: 2018/6/28
     */
    public static Long daysApartReturnTimeStamp(Date start, Date end) {
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
        try {
            Date StartDate = df.parse(df.format(start));
            Date endDate = df.parse(df.format(end));
    ...
    public static Date setDayPlus(String date, int dayAmount) {
        ...
        DateTime dateTime = new DateTime(date);
        if (dayAmount >= 29) {
            Date[] dates = DateUtils.computeMonthFirstAndLastDay(DateUtils.getDate(date), 0);
            String dateStr =DateUtils.getDateToString(dates[1]);
    ...
    public static List<Date> computeDateRangeDate(Date from, Date to) {
        ...
        from = DateUtils.getDateFormat(from, DATE_DEFAULT_FORMAT);
        to = DateUtils.getDateFormat(to, DATE_DEFAULT_FORMAT);
    ...
    public static Date[] computeMonthFirstAndLastDay(Date sourceDate, int monthInterval) {
        ...
        destDates[0] = DateUtils.getDate(DATE_FORMATTER.get().format(cal.getTime()));
        if (monthInterval < 0) {
            cal.setTime(sourceDate);
            if (monthInterval < -1) {
        ...
        destDates[1] =DateUtils.getDate(DATE_FORMATTER.get().format(cal.getTime()));
    ...
    public static Date getRedSpotDate() {
        Date redSpotDate = DateUtils.getDate("2018-2-21");
        return redSpotDate;
    }
...
//        int totalMonth = diff.get(0)*12 + diff.get(1) + (diff.get(2) > 0?1:0);
        ...
        if (DateUtils.getDay(start) <= dayOfMonth) {
            Date[] dates = DateUtils.computeMonthFirstAndLastDay(DateUtils.getDate(start), 0);
            String dateStr = DateUtils.getDateToString(dates[1]);
            String dayStr = dateStr.substring(dateStr.length() - 2);
            Integer day = Integer.valueOf(dayStr);
            if (dayOfMonth <= day) {
                String computeDate = dateStr.substring(0, dateStr.lastIndexOf("-") + 1);
                Date targetDate = DateUtils.getDate(computeDate + dayOfMonth);
                dateList.add(targetDate);
            }
        }
        //如果目标天大于结束日期的天那么只计算到结束日期的上个月
        if (DateUtils.getDay(end) < dayOfMonth) {
            totalMonth--;
        }
        String sourceDate = DateUtils.getDateToString(start);
        for (int i = 0; i < totalMonth; i++) {
            sourceDate = DateUtils.getDateToString(DateUtils.addMonth(sourceDate, 1));
            String computeDate = sourceDate.substring(0, sourceDate.lastIndexOf("-") + 1);
            Date[] dates = DateUtils.computeMonthFirstAndLastDay(DateUtils.getDate(sourceDate), 0);
            String dateStr = DateUtils.getDateToString(dates[1]);
            String dayStr = dateStr.substring(dateStr.length() - 2);
            Integer day = Integer.valueOf(dayStr);
            if (dayOfMonth <= day) {
...
//        int totalMonth = diff.get(0)*12 + diff.get(1) + (diff.get(2) > 0?1:0);
        ...
        if (DateUtils.getDay(start) <= dayOfMonth) {
            ...
            if (dayOfMonth <= day) {
                ...
                Date targetDate = DateUtils.getDate(computeDate);
                dateList.add(targetDate);
            }
        }
        //如果目标天大于结束日期的天那么只计算到结束日期的上个月
     ...
     * @return
     */
    public static long getHourTimeStamp(String str) {
    ...
    public static void main(String[] args) {
        long hourTimeStamp = DateUtils.daysApartReturnTimeStamp(new Date(), DateUtils.addDay(new Date(), 10));
