...
public class UsersServiceImpl extends CrudServiceSupport<UsersMapper, Users> implements UsersService {
    ...
    public Integer saveUsers(UsersSaveDto usersSaveDto) {
        ...
        UsersSaveDto bean = UsersSaveDto.builder()
                .code(i)
                .regTime(timeStamp)
                ...
                .emailValidated(0)
                .messageMask(i)
                .nickname(usersSaveDto.getNickname())
                .mobile(usersSaveDto.getMobile())
                .password(usersSaveDto.getPassword())
                .paypwd(usersSaveDto.getPaypwd())
                .firstLeader(usersSaveDto.getFirstLeader())
                .secondLeader(usersSaveDto.getSecondLeader())
                .firstLeader(usersSaveDto.getThirdLeader())
                .userType(1)//  普通用户
                .build();
    ...
    @Override
    public void modifyAccount(Integer userId, int type, BigDecimal score, int direction, String remark, IncomeTypeEnum incomeTypeEnum) {
        if (type == 2) {
            //  积分增减
    ...
    @Override
    @Transactional(rollbackFor = {Exception.class, RuntimeException.class})
    public boolean modifyUserMoney(Integer userId, BigDecimal score, int direction, String remark, IncomeTypeEnum incomeTypeEnum) {
        ...
        this.updateById(users);
        //  积分操作记录
        AccountLogSaveDto build = AccountLogSaveDto.builder()
                .userId(Long.valueOf(userId))
                .userMoney(new BigDecimal("0"))
                .dogeMoney(0)
                .pigCurrency(0)
                .changeTime(DateUtil.getNowDate())
                .desc(remark).build();
    ...
    public List<Users> convertUserPoints(List<PigReservation> reservats) {
        ...
        for (PigReservation reservat : reservats) {
            ...
            if (CommEnum.FALSE.getCode() == reservat.getReservationStatus().intValue()) {
                ...
                users.add(user);
                accountLogService.convertAndInsert(user.getId(), new BigDecimal("0"), new BigDecimal("0"), reservat.getPayPoints(), new BigDecimal("0"), "抢购失败,退回积分", IncomeTypeEnum.RESERVAT, reservat.getPigId(), "", null);
            }
        }
        return users;
    }
    ...
    @Override
    @Transactional(rollbackFor = {Exception.class, RuntimeException.class})
    public boolean modifyPayPoints(Integer userId, int score, int direction, String remark, IncomeTypeEnum incomeTypeEnum) {
        if (score <= 0) {
    ...
    @Override
    @Transactional(rollbackFor = {Exception.class, RuntimeException.class})
    public MemberLoginResponseDto login(String mobile, String password,
        ...
        MemberLoginResponseDto build = MemberLoginResponseDto.builder().mobile(user.getMobile()).token(user.getToken()).userId(Long.valueOf(user.getId())).build();
        return build;
    }
    ...
    public Long create(MemberLoginRequestDto memberLoginRequestDto) {
        /*//  图形验证码校验
        String graphVerifyCode = RedisUtils.get(memberLoginRequestDto.getCodeKey());
        if (StringUtils.isEmpty(graphVerifyCode)) {
            throw new BizException("图形验证码失效");
        }*/
        //  短信校验
...
//        }
        ...
        //  分销关系建立
        UsersSaveDto build = UsersSaveDto.builder()
                .mobile(memberLoginRequestDto.getMobile())
                .nickname(StringUtils.EMPTY)
                .password(MD5.getMD5Code(memberLoginRequestDto.getPassword()))
                .paypwd(MD5.getMD5Code(memberLoginRequestDto.getPaypwd()))
                .build();
    ...
    public void fgPwd(MemberLoginRequestDto memberLoginRequestDto) {
        Users user = this.getOne(Wrappers.<Users>lambdaQuery().eq(Users::getMobile, memberLoginRequestDto.getMobile()));
        if (user == null) {
        ...
        boolean b = sysSmsFpi.checkSmsCode(memberLoginRequestDto.getMobile(),
                memberLoginRequestDto.getSmsCode(), memberLoginRequestDto.getCodeType());
        if (!b) {
    ...
    public void checkUpgrade(Long leaderId) {
        ...
        for (UserLevel level : levels) {
            ...
            if (users.size() >= level.getStraightPush() && Long.valueOf(u.getLevel()).longValue() != level.getId().longValue()) {
    ...
    public String downUrl(Integer sysType) {
        String downUrl = null;
        if (sysType == 1) {
            Config config = configService.getOne(Wrappers.<Config>lambdaQuery().eq(Config::getConfigName, "ios_link_download"));
            downUrl = config.getConfigValue();
        }
        if (sysType == 2) {
            Config config = configService.getOne(Wrappers.<Config>lambdaQuery().eq(Config::getConfigName, "app_wenjian"));
            downUrl = config.getConfigValue();
        }
        return downUrl;
    }
    ...
    public HomeDto home() {
        ...
        homeDto.setUserIdentity(userIdentityService.count(Wrappers.<UserIdentity>lambdaQuery().eq(UserIdentity::getStatus, 0)));
        ...
        //  今日订单
        homeDto.setTodayOrders(pigOrderService.count(Wrappers.<PigOrder>lambdaQuery().eq(PigOrder::getPayStatus, 2)
                .ge(PigOrder::getEstablishTime, time)));
        //  总会员数
