...
public class OrderServiceImpl implements OrderService {
    ...
    public BaseResponse createOrder(OrderTypeEnum orderTypeEnum, Object param) {
        ...
        OrderModelServer modelServer = orderModelServers.stream()
                .filter(item -> item.isSupport(orderTypeEnum))
                .findFirst()
...
//        pigManager.addSellIncomePig(pigs);  精灵实例在出售收益时生成
        ...
        for (Long notLuckUserId : notLuckUsers) {
            ...
            accountLogService.convertAndInsert(notLuckUserId, new BigDecimal("0"), new BigDecimal("0"), goods.getAdoptiveEnergy(), new BigDecimal("0"), "抢购失败,退回积分", IncomeTypeEnum.RESERVAT, goods.getId(), "", null);
        }
